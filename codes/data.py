...
class TrainDataset(Dataset):
    ...
    def calculate_cosine_distances(self, head, rel, tail):
        pos_embedding = self.get_embedding(head, rel, tail)
        entity_embeddings = self.get_all_entity_embeddings()
        distances = torch.cosine_similarity(pos_embedding.unsqueeze(0), entity_embeddings)
        return distances
